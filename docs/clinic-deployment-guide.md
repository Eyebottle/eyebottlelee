...
2. **마이크 점검 민감도 과도**: 조용한 진료실에서 일반적인 음성 수준으로 발화했음에도 마이크 점검 카드가 "입력이 약함"(-42 dBFS, SNR -1.3 dB)으로 표시됩니다. 2025-09-27 두 번째 패치에서 정상 판정 기준을 -40 dB / SNR 3.5 dB로 완화하고, -48 dB · SNR -0.5 dB 이상이면 "녹음은 가능" 메시지로 안내하도록 조정했습니다. 현장 재테스트로 확인 예정.
