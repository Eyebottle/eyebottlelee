...
2. **마이크 점검 민감도 과도**: 조용한 진료실에서 일반적인 음성 수준으로 발화했음에도 마이크 점검 카드가 "입력이 약함"(-42 dBFS, SNR -1.3 dB)으로 표시됩니다. 2025-09-27 패치에서 정상 판정 기준을 -38 dB / SNR 4 dB로 완화하고, 매우 조용한 환경에서도 -45 dB · SNR -1.5 dB 이상이면 "정상"으로 처리하도록 조정했습니다. 현장 재테스트로 확인 예정.
