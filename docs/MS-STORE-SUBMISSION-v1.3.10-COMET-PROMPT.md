...
- Removed PowerShell/WMIC dependency for MSIX sandbox compatibility

TEST:
1. Settings > Windows Startup > Toggle OFF > Restart > App should NOT start
2. Toggle startup ON + background ON > Restart > App in tray only (no window)
3. Toggle startup ON + background OFF > Restart > Window appears normally

All v1.3.4 features unchanged. Requires: Windows 10 1809+, VCLibs.140.00.UWPDesktop
